...
class CAQModel(nn.Module):
    def __init__(self, tda_model, w_emb, q_emb, v_att, q_net, v_net, updated_query_composer, neighbour_attention, Dropout_C, classifier, dataset):
    ...
    def forward(self, v, b, q, labels, mask):
        ...
        """
        baseline_out = self.tda_model.forward_hiddenrep(v, b, q, labels)
        ...
        cur_group = baseline_out.contiguous().view(mask.size(0), -1, baseline_out.size(-1))
...
def build_caq_newatt(dataset, num_hid, tda_model):
    w_emb = WordEmbedding(dataset.dictionary.ntoken, 300, 0.0)
    q_emb = QuestionEmbedding(300, num_hid, 1, False, 0.0)
    v_att = Attention(dataset.v_dim, q_emb.num_hid, num_hid)
    ...
    return CAQModel(tda_model, w_emb, q_emb, v_att, q_net, v_net, updated_query_composer, neighbour_attention, Dropout_C, classifier, dataset)
