...
def _get_maxq_count(dict):
    print('total image count :', len(dict))
    max_q = 0
...
def _load_dataset(dataroot, name, img_id2val):
    ...
    for question, answer in zip(questions, answers):
        ...
        if img_id not in image2q:
            image2q[img_id] = [entry_id]
        else:
            image2q[img_id].append(entry_id)

        entry_id += 1

    return entries, image2q
...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.stack(tot_features,0), torch.stack(tot_spatials,0), torch.stack(tot_questions,0), torch.stack(tot_targets,0), torch.tensor(tot_q_ids)
...
class VQAFeatureDataset_withmask(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.stack(tot_features,0), torch.stack(tot_spatials,0), torch.stack(tot_questions,0), \
               torch.stack(tot_targets,0), adj.type(torch.FloatTensor), torch.tensor(tot_q_ids)
